...
import com.springbank.account.cmd.api.commands.RestoreReadDbCommand;
...
public class RestoreReadDbController {
    ...
    public ResponseEntity<BaseResponse> restoreReadDb() {
        try {
            ...
            return new ResponseEntity<>(new BaseResponse("Read database restore request completed successfully!"),
                    HttpStatus.CREATED);
        } catch (IllegalStateException e) {
            logger.log(Level.INFO, MessageFormat.format("Client made a bad request - {0}.", e.toString()));
            return new ResponseEntity<>(new BaseResponse(e.toString()), HttpStatus.BAD_REQUEST);
        } catch (Exception e) {
            var safeErrorMessage = "Error while processing request to restore read database - {0}";
